...
console.warn(
  "[DEPRECATED] context-periodic-table.js is deprecated. Use context-taxonomy.js instead.",
);
...
export const GROUPS = {
  0: {
    name: "Noble",
    description: "Stable, autonomous contexts",
    color: "#FFD700",
  },
  1: {
    name: "Development",
    description: "Code-focused contexts",
    color: "#3B82F6",
  },
  ...
  6: {
    name: "Management",
    description: "Project and team leadership",
    color: "#22C55E",
  },
  7: {
    name: "Business",
  ...
  "-1": {
    name: "Synthetic",
    description: "Created through reactions",
    color: "#888888",
  },
};
...
export const BLOCKS = {
  ...
  p: {
    name: "Operational",
    description: "Infrastructure and operational focus",
  },
  d: {
    name: "Transitional",
    description: "Bridge between technical and business",
  },
  f: { name: "Specialized", description: "Deep domain expertise" },
  noble: { name: "Noble", description: "Stable and autonomous" },
  synthetic: {
    name: "Synthetic",
    description: "Created through context reactions",
  },
};
...
export function classifyContext(profile) {
  ...
  for (const [key, element] of Object.entries(CONTEXT_PERIODIC_TABLE)) {
    ...
    // Competency overlap
    const elementComps = new Set(
      element.typicalCompetencies.map((c) => c.toLowerCase()),
    );
    const overlap = [...competencySet].filter((c) =>
      elementComps.has(c),
    ).length;
    score += overlap * 2;
...
export function getCompatiblePartners(element, allContexts) {
  ...
  for (const context of allContexts) {
    ...
    if (
      element.block === "d" &&
      (otherElement.block === "s" || otherElement.block === "p")
    ) {
      compatible.push({
        context,
        element: otherElement,
...
export function generatePeriodicTableView() {
  ...
  for (let period = 1; period <= maxPeriod; period++) {
    ...
    for (let group = 0; group <= maxGroup; group++) {
      const element = Object.values(CONTEXT_PERIODIC_TABLE).find(
        (e) =>
          e.period === period && e.group === group && e.block !== "synthetic",
      );
  ...
  const syntheticRow = Object.values(CONTEXT_PERIODIC_TABLE).filter(
    (e) => e.block === "synthetic",
  );
...
export function getDiscoveryStatus(userContexts) {
  ...
  const allElements = Object.values(CONTEXT_PERIODIC_TABLE).filter(
    (e) => e.block !== "synthetic",
  );
  ...
  return {
    ...
    completionPercentage: Math.round(
      (discovered.size / allElements.length) * 100,
    ),
