...
export function classifyContext(profile) {
  ...
  if (profile.chitty_id_type === "S")
    return {
      type: CONTEXT_TYPES.SUPERNOVA,
      confidence: 1.0,
      reason: "Created by fusion",
    };
  if (profile.chitty_id_type === "F")
    return {
      type: CONTEXT_TYPES.FISSION,
      confidence: 1.0,
      reason: "Created by fission",
    };
  if (profile.chitty_id_type === "D")
    return {
      type: CONTEXT_TYPES.DERIVATIVE,
      confidence: 1.0,
      reason: "Created as derivative",
    };
  if (profile.chitty_id_type === "X")
    return {
      type: CONTEXT_TYPES.SUSPENSION,
      confidence: 1.0,
  ...
  for (const [key, type] of Object.entries(CONTEXT_TYPES)) {
    ...
    const typeComps = new Set(
      type.indicators.competencies.map((c) => c.toLowerCase()),
    );
    ...
    const typeDomains = new Set(
      type.indicators.domains.map((d) => d.toLowerCase()),
    );
  ...
  if (scores.length > 0) {
    const best = scores[0];
    const maxPossible =
      best.type.indicators.competencies.length * 2 +
      best.type.indicators.domains.length * 3;
    ...
    return {
      ...
      alternatives: scores.slice(1, 3).map((s) => ({
        type: s.type,
        confidence:
          Math.round((s.score / Math.max(maxPossible, 1)) * 100) / 100,
      })),
    };
  }
...
export function getDiscoveryStatus(userContexts) {
  ...
  const allTypes = Object.values(CONTEXT_TYPES).filter(
    (t) => t.category !== "synthetic" && t.id !== "general",
  );
