...
async function mintChittyId(env, entityType, metadata = {}) {
  ...
  try {
    ...
    console.warn('[ContextIntelligence] ChittyID service returned invalid response, using fallback');
  } catch (error) {
    console.warn('[ContextIntelligence] ChittyID service unavailable, using fallback:', error.message);
  }
  ...
  const checksum = '0';
  const variant = Math.floor(Math.random() * 100).toString().padStart(2, '0');
...
export class ContextIntelligence {
  constructor(env) {
    ...
    this.toolRegistry = {
      'wrangler': { competencies: ['cloudflare-workers', 'deployment'], domains: ['backend-development', 'infrastructure'] },
      'd1': { competencies: ['d1', 'sql', 'database'], domains: ['backend-development', 'data'] },
      'vectorize': { competencies: ['ai', 'embeddings', 'vector-search'], domains: ['ai-development', 'search'] },
      'typescript': { competencies: ['typescript', 'javascript'], domains: ['frontend-development', 'backend-development'] },
      'git': { competencies: ['git', 'version-control'], domains: ['development'] },
      'chittyid_mint': { competencies: ['identity', 'chittyos'], domains: ['identity-management'] },
      'chitty_evidence_ingest': { competencies: ['evidence', 'legal'], domains: ['legal', 'evidence-management'] },
      'chitty_case_create': { competencies: ['legal', 'case-management'], domains: ['legal'] },
      'chitty_finance_connect': { competencies: ['finance', 'banking'], domains: ['financial'] },
      'notion_query': { competencies: ['notion', 'documentation'], domains: ['documentation', 'project-management'] },
      'neon_query': { competencies: ['postgresql', 'sql', 'database'], domains: ['backend-development', 'data'] },
    };

    // Risk levels for operations
    this.operationRisks = {
      'read': 0, 'write': 1, 'delete': 3,
      'deploy_staging': 2, 'deploy_production': 4,
      'secret_access': 4, 'financial_transaction': 5,
      'legal_filing': 5, 'permission_change': 4, 'data_export': 3,
    };
    ...
    this.autonomyThresholds = {
      ...
      4: { maxRisk: 3, autoApprove: ['read', 'write', 'deploy_staging', 'delete'] },
      5: { maxRisk: 4, autoApprove: ['read', 'write', 'deploy_staging', 'deploy_production', 'delete', 'data_export'] },
    };
  }
  ...
  async analyzeContextCoherence(boundChittyId, currentHints) {
    ...
    if (!profile) return { recommendation: 'error', reason: 'Context not found' };
    ...
    const alternatives = await this.findBetterContexts(currentHints, boundChittyId);

    if (coherenceScore >= 0.7) {
      return { recommendation: 'continue', coherenceScore, reason: 'Work aligns with context' };
    } else if (coherenceScore >= 0.4 && driftIndicators.isExpansion) {
      return { recommendation: 'expand', coherenceScore, expansionAreas: driftIndicators.newDomains, confirm: true };
    } else if (alternatives.length > 0 && alternatives[0].score > coherenceScore + 0.2) {
      return { recommendation: 'switch', coherenceScore, suggestedContext: alternatives[0] };
    } else if (coherenceScore < 0.3) {
      return { recommendation: 'new', coherenceScore, reason: 'Work differs significantly' };
    }
    return { recommendation: 'confirm', coherenceScore, driftIndicators, alternatives };
  }
  ...
  calculateCoherence(profile, hints) {
    ...
    if (hints.projectPath) {
      ...
      else if (this.pathsRelated(profile.project_path, hints.projectPath)) score += 1.5;
    }
    if (hints.workspace) { factors += 1; if (profile.workspace === hints.workspace) score += 1; }
    if (hints.supportType) { factors += 2; if (profile.support_type === hints.supportType) score += 2; }
    return factors > 0 ? score / factors : 0.5;
  }

  detectDriftIndicators(profile, hints) {
    const existingDomains = new Set(profile.expertise_domains);
    const newDomains = (hints.domains || []).filter(d => !existingDomains.has(d));
    const projectChanged = hints.projectPath && profile.project_path !== hints.projectPath;
    return {
      newDomains, projectChanged,
      isExpansion: !projectChanged && newDomains.length > 0 && newDomains.length <= 2,
  ...
  async findBetterContexts(hints, excludeChittyId) {
    const results = await this.db.prepare(`
      ...
      LIMIT 10
    `).bind(excludeChittyId, hints.projectPath || '', hints.supportType || '').all();

    return results.results.map(ctx => ({
  ...
  suggestTools(profile) {
    const competencyNames = new Set(profile.competencies.map(c => typeof c === 'string' ? c : c.name));
    ...
    for (const [tool, req] of Object.entries(this.toolRegistry)) {
      ...
      if (compMatch || domMatch) {
        suggestions.push({ tool, relevance: (compMatch ? 0.6 : 0) + (domMatch ? 0.4 : 0) });
      }
    }
  ...
  determineAutonomy(profile) {
    ...
    if (profile.success_rate > 0.9 && profile.total_decisions > 50) maxRisk = Math.min(5, maxRisk + 1);

    const autoApprove = Object.entries(this.operationRisks)
      .filter(([, risk]) => risk <= maxRisk).map(([op]) => op);
    const requiresConfirm = Object.entries(this.operationRisks)
      .filter(([, risk]) => risk > maxRisk).map(([op, risk]) => ({ operation: op, risk }));
  ...
  determineGuardrails(profile) {
    const guardrails = [];
    if (profile.trust_level < 2) guardrails.push({ type: 'confirmation_required', scope: 'all_writes' });
    if (profile.anomaly_count > 3) guardrails.push({ type: 'enhanced_logging', scope: 'all_operations' });
    if (profile.success_rate < 0.5 && profile.total_decisions > 10) {
      guardrails.push({ type: 'additional_validation', scope: 'complex_operations' });
    }
    return guardrails;
  }
  ...
  determineRouting(profile) {
    const routes = {
      'development': 'chittyconnect', 'operations': 'chittymonitor',
      'legal': 'chittycases', 'financial': 'chittyfinance',
    };
  ...
  async findCollaborators(projectHints, requiredCompetencies = []) {
    ...
    for (const ctx of results.results) {
      ...
      const compNames = competencies.map(c => typeof c === 'string' ? c : c.name);
      const matched = requiredCompetencies.filter(r => compNames.includes(r));
      if (matched.length > 0) {
        candidates.push({
          chittyId: ctx.chitty_id, supportType: ctx.support_type,
          trustLevel: ctx.trust_level, matchedCompetencies: matched,
  ...
  async createCollaboration(parentChittyId, childChittyId, projectId, scope, permissions) {
    ...
    if (parent.trust_level < 3) return { error: 'Parent needs trust level 3+ to delegate' };
    ...
    `).bind(id, parent.id, parentChittyId, child.id, childChittyId, projectId,
      JSON.stringify(scope), JSON.stringify(permissions)).run();
  ...
  async createContextPair(chittyId1, chittyId2, relationship) {
    ...
    const comp1 = new Set(ctx1.competencies.map(c => typeof c === 'string' ? c : c.name));
    const comp2 = new Set(ctx2.competencies.map(c => typeof c === 'string' ? c : c.name));
    const overlap = [...comp1].filter(c => comp2.has(c));
    const unique1 = [...comp1].filter(c => !comp2.has(c));
    const unique2 = [...comp2].filter(c => !comp1.has(c));

    const complementarity = unique1.length + unique2.length > overlap.length ? 'complementary' : 'overlapping';
    ...
    await this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', unixepoch())
    `).bind(pairId, ctx1.id, chittyId1, ctx2.id, chittyId2, relationship,
      complementarity, JSON.stringify(overlap), JSON.stringify(unique1), JSON.stringify(unique2)).run();

    return {
      pairId, relationship, complementarity,
      overlap, unique1, unique2,
      recommendation: complementarity === 'complementary'
  ...
  async analyzeSupernova(chittyId1, chittyId2) {
    ...
    const relatedProjects = this.pathsRelated(ctx1.project_path, ctx2.project_path);
    const trustDiff = Math.abs(ctx1.trust_level - ctx2.trust_level);
    const totalAnomalies = (ctx1.anomaly_count || 0) + (ctx2.anomaly_count || 0);

    const risks = [];
    if (trustDiff > 2) risks.push({ type: 'trust_dilution', severity: 'high' });
    if (!sameSupport) risks.push({ type: 'role_conflict', severity: 'medium' });
    if (totalAnomalies > 5) risks.push({ type: 'anomaly_accumulation', severity: 'high' });
    if (!relatedProjects) risks.push({ type: 'identity_confusion', severity: 'high' });

    // Merged preview
    const comp1 = ctx1.competencies.map(c => typeof c === 'string' ? c : c.name);
    const comp2 = ctx2.competencies.map(c => typeof c === 'string' ? c : c.name);
    const mergedCompetencies = [...new Set([...comp1, ...comp2])];
    const mergedDomains = [...new Set([...ctx1.expertise_domains, ...ctx2.expertise_domains])];

    const riskLevel = risks.filter(r => r.severity === 'high').length > 1 ? 'critical'
      : risks.length > 2 ? 'warning' : 'info';

    return {
      contexts: [{ chittyId: chittyId1, supportType: ctx1.support_type, trustLevel: ctx1.trust_level },
                 { chittyId: chittyId2, supportType: ctx2.support_type, trustLevel: ctx2.trust_level }],
      risks, riskLevel,
      mergedPreview: { competencies: mergedCompetencies, domains: mergedDomains,
        trustLevel: Math.min(ctx1.trust_level, ctx2.trust_level) },
      recommendation: riskLevel === 'critical' ? 'discourage' : riskLevel === 'warning' ? 'caution' : 'consider',
    };
  }
  ...
  async executeSupernova(chittyId1, chittyId2, confirmationToken) {
    ...
    if (analysis.recommendation === 'discourage') return { error: 'Supernova discouraged', analysis };
    ...
    `).bind(mergedId, mergedChittyId, `supernova:${ctx1.context_hash}:${ctx2.context_hash}`,
      ctx1.project_path, ctx1.workspace, ctx1.support_type, ctx1.organization,
      `supernova:${chittyId1}+${chittyId2}`,
      (ctx1.trust_score + ctx2.trust_score) / 2, Math.min(ctx1.trust_level, ctx2.trust_level)).run();
    ...
    await this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(crypto.randomUUID(), mergedId, mergedChittyId,
      ...
      (ctx1.anomaly_count || 0) + (ctx2.anomaly_count || 0)).run();

    // Archive source contexts
    await this.db.prepare(`UPDATE context_entities SET status = 'archived' WHERE chitty_id IN (?, ?)`)
      .bind(chittyId1, chittyId2).run();

    return { success: true, mergedChittyId, sourceContexts: [chittyId1, chittyId2], analysis };
  }
  ...
  async analyzeFission(chittyId, splitCriteria) {
    ...
    if (splitBy === 'domain') {
      ...
      proposedSplits = [
        { supportType: 'development', keep: ['typescript', 'javascript', 'react', 'api-design'] },
        { supportType: 'operations', keep: ['deployment', 'monitoring', 'infrastructure'] },
      ];
    }

    return {
      sourceContext: { chittyId, supportType: ctx.support_type, trustLevel: ctx.trust_level },
      proposedSplits,
  ...
  async executeFission(chittyId, splitConfig, confirmationToken) {
    ...
    for (const split of splitConfig.splits) {
      ...
      `).bind(newId, newChittyId, `fission:${ctx.context_hash}:${split.label}`,
        ctx.project_path, ctx.workspace, split.supportType || ctx.support_type,
        ctx.organization, `fission:${chittyId}`, ctx.trust_score, ctx.trust_level).run();

      await this.db.prepare(`
        INSERT INTO context_dna (id, context_id, context_chitty_id, competencies, expertise_domains)
        VALUES (?, ?, ?, ?, ?)
      `).bind(crypto.randomUUID(), newId, newChittyId,
        JSON.stringify(split.competencies || []), JSON.stringify(split.domains || [])).run();

      newContexts.push({ chittyId: newChittyId, label: split.label });
    }

    // Archive source
    await this.db.prepare(`UPDATE context_entities SET status = 'archived' WHERE chitty_id = ?`)
      .bind(chittyId).run();
  ...
  async createDerivative(sourceChittyId, derivativeConfig) {
    ...
    `).bind(
      derivativeId, derivativeChittyId,
    ...
    await this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `).bind(
      crypto.randomUUID(), derivativeId, derivativeChittyId,
  ...
  async createSuspension(contextIds, suspensionConfig) {
    ...
    if (contextIds.length < 2) return { error: 'Need at least 2 contexts for suspension' };
    ...
    for (const ctx of contexts) {
      for (const comp of ctx.competencies) {
        ...
        if (!allCompetencies.has(name)) allCompetencies.set(name, { name, level: 1, count: 0, sources: [] });
        const existing = allCompetencies.get(name);
        existing.level = Math.max(existing.level, typeof comp === 'string' ? 1 : (comp.level || 1));
        existing.count++;
    ...
    `).bind(
      suspensionId, suspensionChittyId,
    ...
    await this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `).bind(
      crypto.randomUUID(), suspensionId, suspensionChittyId,
    ...
    `).bind(
      ...
      JSON.stringify({ taskDescription, expiresAt: Math.floor(Date.now() / 1000) + expiresIn }),
      taskDescription
    ).run();
  ...
  async dissolveSuspension(suspensionChittyId, rollbackMetrics = true) {
    ...
    `).bind(crypto.randomUUID(), JSON.stringify([suspensionChittyId]), JSON.stringify(sourceIds)).run();
  ...
  async createSolution(contextIds, solutionConfig) {
    ...
    if (contextIds.length < 2) return { error: 'Need at least 2 contexts for a solution' };
    ...
    for (const chittyId of contextIds) {
      ...
      const role = roles[chittyId] || this.inferRole(
      ...
      members.push({
        ...
        strengths: profile.competencies.slice(0, 3).map(c => typeof c === 'string' ? c : c.name),
      });
    }
  ...
  async createCombination(chittyId1, chittyId2, combinationConfig = {}) {
    const { shareDirection = 'bidirectional', shareDomains = true, shareCompetencies = true } = combinationConfig;
    ...
    `).bind(
      combinationId, ctx1.id, chittyId1, ctx2.id, chittyId2,
    ...
    if (shareDomains && (shareDirection === 'bidirectional' || shareDirection === '1to2')) {
      await this.crossPollinateDomains(ctx1, ctx2);
    }
    if (shareDomains && (shareDirection === 'bidirectional' || shareDirection === '2to1')) {
  ...
  async crossPollinateDomains(fromCtx, toCtx) {
    ...
    const toDna = await this.db.prepare(`SELECT expertise_domains FROM context_dna WHERE context_chitty_id = ?`)
      .bind(toCtx.chitty_id).first();

    if (!toDna) return;

    const currentDomains = JSON.parse(toDna.expertise_domains || '[]');
    const newDomains = [...new Set([...currentDomains, ...fromCtx.expertise_domains])];

    await this.db.prepare(`UPDATE context_dna SET expertise_domains = ? WHERE context_chitty_id = ?`)
      .bind(JSON.stringify(newDomains), toCtx.chitty_id).run();
  }
  ...
  summarizeProfile(profile) {
    return {
      chittyId: profile.chitty_id, projectPath: profile.project_path,
      supportType: profile.support_type, trustLevel: profile.trust_level,
      totalInteractions: profile.total_interactions,
      successRate: profile.success_rate ? Math.round(profile.success_rate * 100) : null,
      competencies: profile.competencies.slice(0, 5).map(c => typeof c === 'string' ? c : c.name),
    };
  }
  ...
  groupDomains(domains) {
    ...
    const devDomains = domains.filter(d => d.includes('development') || d.includes('api'));
    const opsDomains = domains.filter(d => d.includes('operations') || d.includes('infrastructure'));
    const otherDomains = domains.filter(d => !devDomains.includes(d) && !opsDomains.includes(d));
    return [devDomains, opsDomains, otherDomains].filter(g => g.length > 0);
  }

  competencyMatchesDomains(competency, domains) {
    const mappings = {
      'typescript': ['development', 'frontend-development', 'backend-development'],
      'deployment': ['operations', 'infrastructure'],
      'kubernetes': ['operations', 'infrastructure'],
    };
    return (mappings[competency] || []).some(d => domains.some(dom => dom.includes(d)));
  }
}
