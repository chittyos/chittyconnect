...
export const ALCHEMY = {
  ...
  REACTIONS: {
    ...
    PRECIPITATION: {
      ...
      description: 'New context crystallizes from parent, inheriting properties',
      energy: 'neutral',
      chittyIdType: 'D',
      reversible: false,
    ...
    DISSOLUTION: {
      ...
      description: 'Context breaks down, insights optionally absorbed by others',
      energy: 'exothermic',
      chittyIdType: null,
      reversible: false,
...
export function suggestReactions(profile1, profile2 = null) {
  ...
  if (!profile2) {
    ...
    const comp1 = new Set((profile1.competencies || []).map(c => typeof c === 'string' ? c : c.name));
    const comp2 = new Set((profile2.competencies || []).map(c => typeof c === 'string' ? c : c.name));
    ...
    if ((element1.symbol === 'Au' && element2.symbol === 'Na') ||
...
export function describeChittyIdType(typeCode) {
  ...
  return descriptions[typeCode] || { name: 'Unknown', origin: 'Unknown origin' };
}
