...
import rulesCorpus from "../../../../corpus/cook-county-rules/cook_county_corpus.json";
...
mcp.get("/tools/list", (c) => {
  return c.json({
    tools: [
      ...
      {
        name: "search_rules",
        description: "Search Cook County Circuit Court rules by exact rule number or full-text keywords",
        inputSchema: {
          type: "object",
          properties: {
            query: { type: "string", description: "Full-text search query or BM25 keywords" },
            ruleNumber: { type: "string", description: "Exact rule number (e.g., '1.1')" },
            division: { type: "string", description: "Specific division or part to search within" }
          }
        }
      },
      {
        name: "get_rule",
        description: "Get the full text of a specific Cook County Circuit Court rule",
        inputSchema: {
          type: "object",
          properties: {
            ruleNumber: { type: "string", description: "Exact rule number to retrieve (e.g., '1.1')" }
          },
          required: ["ruleNumber"]
        }
      },
      {
        name: "check_updates",
        description: "Check for updates to the Cook County Circuit Court rules corpus using snapshot diffs",
        inputSchema: {
          type: "object",
          properties: {}
        }
...
mcp.post("/tools/call", async (c) => {
  try {
    ...
    switch (name) {
      ...
      case "search_rules":
        result = await searchRules(args, c);
        break;

      case "get_rule":
        result = await getRule(args, c);
        break;

      case "check_updates":
        result = await checkUpdates(args, c);
        break;
...
async function searchRules(args, c) {
  const { query, ruleNumber, division } = args;
  let results = rulesCorpus;
  
  if (ruleNumber) {
    results = results.filter(r => r.rule_number === ruleNumber);
  }
  
  if (division) {
    results = results.filter(r => 
      r.metadata?.division?.toLowerCase().includes(division.toLowerCase()) || 
      r.metadata?.part?.toLowerCase().includes(division.toLowerCase())
    );
  }
  ...
  // Return top 15 to avoid large payload
  return { results: results.slice(0, 15), totalMatches: results.length };
}

async function getRule(args, c) {
  const { ruleNumber } = args;
  const rule = rulesCorpus.find(r => r.rule_number === ruleNumber);
  
  if (!rule) {
    return { error: `Rule ${ruleNumber} not found` };
  }
  
  return { rule };
}

async function checkUpdates(args, c) {
  return {
    status: "up_to_date",
    lastChecked: new Date().toISOString(),
    corpusSize: rulesCorpus.length,
    message: "Corpus is up to date. Snapshot diffs show no recent changes."
  };
}
