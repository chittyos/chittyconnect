...
describe('Context MCP Tools', () => {
  ...
  const env = {};
  const opts = { baseUrl: 'https://connect.chitty.cc', authToken: 'test-token' };

  describe('context_resolve', () => {
    it('should POST to /api/v1/intelligence/context/resolve', async () => {
      const result = await dispatchToolCall('context_resolve', {
        project_path: '/test', platform: 'claude_code', support_type: 'development',
      }, env, opts);
  ...
  describe('context_restore', () => {
    it('should GET restore endpoint with chitty_id and project', async () => {
      await dispatchToolCall('context_restore', {
        chitty_id: '03-1-USA-1234-P-2602-0-01', project_slug: 'myproject',
      }, env, opts);
  ...
  describe('context_commit', () => {
    it('should POST session metrics to commit endpoint', async () => {
      await dispatchToolCall('context_commit', {
        session_id: 'sess-1', chitty_id: '03-1-USA-1234-P-2602-0-01',
  ...
  describe('context_check', () => {
    it('should GET check endpoint with chitty_id', async () => {
      await dispatchToolCall('context_check', {
  ...
  describe('context_checkpoint', () => {
    it('should POST checkpoint data', async () => {
      await dispatchToolCall('context_checkpoint', {
        chitty_id: '03-1-USA-1234-P-2602-0-01', project_slug: 'test', name: 'v1',
      }, env, opts);
      expect(fetch).toHaveBeenCalledWith(
  ...
  describe('memory_persist (enhanced)', () => {
    it('should include chitty_id in request body', async () => {
      await dispatchToolCall('memory_persist', {
        content: 'test', chitty_id: '03-1-USA-1234-P-2602-0-01',
      }, env, opts);
  ...
  describe('memory_recall (enhanced)', () => {
    it('should include chitty_id as query param', async () => {
      await dispatchToolCall('memory_recall', {
        query: 'test', chitty_id: '03-1-USA-1234-P-2602-0-01',
      }, env, opts);
      const url = fetch.mock.calls[0][0];
