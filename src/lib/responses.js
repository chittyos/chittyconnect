...
export function contextualResponse(c, data, options = {}) {
  ...
  const response = {
    ...
    metadata: {
      ...
      processingTime: options.processingTime || (Date.now() - (c.get('startTime') || Date.now())),
...
export function errorResponse(c, error) {
  ...
  const errorMap = {
    'INVALID_ENTITY_TYPE': {
      ...
      recovery: "Check the entity type and try again with a valid value from the list.",
      statusCode: 400
    },
    'RATE_LIMITED': {
      message: "You've made too many requests. Please wait a moment before trying again.",
      recovery: "Wait 60 seconds before making another request. Consider batching multiple operations.",
      statusCode: 429
    },
    'SERVICE_UNAVAILABLE': {
      message: "The service is temporarily unavailable. Our team has been notified.",
      recovery: "Try again in a few minutes. Check service status at /api/services/status.",
      statusCode: 503
    },
    'UNAUTHORIZED': {
      message: "Authentication required. Please provide a valid API key or bearer token.",
      recovery: "Include 'Authorization: Bearer <token>' header in your request.",
      statusCode: 401
    },
    'FORBIDDEN': {
      message: "You don't have permission to perform this action.",
      recovery: "Request additional scopes or contact support to upgrade your access level.",
      statusCode: 403
    },
    'NOT_FOUND': {
      message: "The requested resource was not found.",
      recovery: "Verify the ID or path and try again. Use search endpoints to find resources.",
      statusCode: 404
    },
    'VALIDATION_ERROR': {
    ...
    'CHITTYID_CREATION_FAILED': {
      message: "Failed to create ChittyID. The identity service may be experiencing issues.",
      recovery: "Try again in a moment. If the problem persists, contact support.",
      statusCode: 500
    },
    ...
    'COMPOSITE_OPERATION_FAILED': {
      ...
      recovery: "Review which operations succeeded and retry the failed portions individually.",
      statusCode: 500
    },
    'BATCH_TOO_LARGE': {
      message: "The batch contains too many requests.",
      recovery: "Split the batch into smaller chunks (maximum 10 requests per batch).",
      statusCode: 400
    }
  };
  ...
  const errorInfo = errorMap[error.code] || {
    ...
    recovery: "Please try again. If the issue persists, contact support with the request ID.",
 ...
 * @returns {Response} Hono JSON response
 */
export function successResponse(c, data, message = 'Operation completed successfully', status = 200) {
  return c.json({
    success: true,
    message,
    data,
...
export function partialSuccessResponse(c, succeeded, failed) {
  ...
  }, allSucceeded ? 200 : 207); // 207 Multi-Status
}