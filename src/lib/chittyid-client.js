...
export class ChittyIDClient {
   ...
   */
  constructor(options = {}) {
    this.serviceUrl = options.serviceUrl || process.env.CHITTYID_SERVICE_URL || DEFAULT_SERVICE_URL;
  ...
  async mint(entityType, options = {}) {
    ...
    if (options.trust !== undefined) params.set('trust', options.trust.toString());
    ...
    if (!response.ok) {
      ...
      throw new ChittyIDError(error.error || 'MINT_FAILED', error.message || response.statusText);
    }
  ...
  async validate(chittyId) {
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({ error: 'Unknown error' }));
      throw new ChittyIDError(error.error || 'VALIDATION_FAILED', error.message || response.statusText);
    }
  ...
  async getSpec() {
    ...
    if (!response.ok) {
      throw new ChittyIDError('SPEC_FAILED', 'Failed to retrieve specification');
    }
