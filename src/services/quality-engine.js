...
export const QualityRules = {
  draftPatterns: [
    /draft/i, /\(draft\)/i, /v\d+\.\d+/i, /temp/i, /test/i,
    /scratch/i, /untitled/i, /copy of/i, /- Copy/i, /\d{8}_/,
    /\(1\)/i, /\(2\)/i, /wip/i, /todo/i, /notes/i,
  ],
  junkContentPatterns: [
    /lorem ipsum/i, /test test test/i, /asdfasdf/i,
    /\[INSERT .+\]/i, /\[TODO\]/i, /\[TK\]/i,
  ],
  ...
  incompletePatterns: [
    /\[REDACTED\]/i, /\[CLIENT NAME\]/i, /\[DATE\]/i,
    /\[JURISDICTION\]/i, /YOUR NAME HERE/i, /\{\{.+\}\}/,
  ],
};
...
export class ChittyQualityEngine {
  ...
  async validateDocument(file) {
    ...
    const { confidence, recommendation, reasoning } = this.calculateRecommendation(issues);
