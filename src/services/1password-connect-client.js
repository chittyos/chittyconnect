...
OnePasswordConnectClient.prototype.put = async function (
  ...
  const itemsResp = await fetch(
    `${this.connectUrl}/v1/vaults/${parsed.vaultId}/items`,
    { headers },
  );
  if (!itemsResp.ok)
    throw new Error(`Failed to list items: ${itemsResp.status}`);
  ...
  if (existing) {
    ...
    const detailResp = await fetch(
      `${this.connectUrl}/v1/vaults/${parsed.vaultId}/items/${existing.id}`,
      { headers },
    );
    if (!detailResp.ok)
      throw new Error(`Failed to get item: ${detailResp.status}`);
    const itemDetails = await detailResp.json();

    let fieldFound = false;
    for (const f of itemDetails.fields || []) {
      if (f.label?.toLowerCase() === parsed.field.toLowerCase()) {
        f.value = value;
        fieldFound = true;
        break;
      }
    ...
    if (!updateResp.ok) {
      ...
      throw new Error(`Update failed: ${updateResp.status} ${err}`);
    }
      ...
      fields: [
        {
          ...
          value:
            options.notes ||
            `Stored via ChittyConnect ${new Date().toISOString()}`,
        },
    ...
    if (!createResp.ok) {
      ...
      throw new Error(`Create failed: ${createResp.status} ${err}`);
    }
