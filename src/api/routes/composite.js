...
const CompleteCaseSchema = z.object({
  ...
  parties: z.array(z.object({
    role: z.enum(['plaintiff', 'defendant', 'witness', 'attorney', 'judge', 'other']),
    name: z.string(),
  ...
  initialEvidence: z.record(z.object({
    type: z.enum(['document', 'photo', 'video', 'audio', 'financial', 'communication', 'other']),
 ...
 */
compositeRoutes.post('/case-with-parties',
  ...
  async (c) => {
    ...
    try {
      ...
      const chittyEvidenceToken = await getServiceToken(c.env, 'chittyevidence');
      ...
      if (input.parties && input.parties.length > 0) {
        ...
        operations.push(...partyResults.map(p => ({
          type: 'chittyid',
          id: p.chittyId,
          service: 'chittyid'
        })));
      ...
      let evidenceResults = [];
      if (input.initialEvidence && Object.keys(input.initialEvidence).length > 0) {
        const evidencePromises = Object.entries(input.initialEvidence).map(
          async ([key, evidence]) => {
            // Create ChittyID for evidence
            const evidenceIdResponse = await fetch('https://id.chitty.cc/v1/mint', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${chittyIdToken}`
            ...
            if (chittyEvidenceToken) {
              const ingestResponse = await fetch('https://evidence.chitty.cc/v1/ingest', {
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
        ...
        operations.push(...evidenceResults.map(e => ({
          type: 'evidence',
          id: e.id,
...
compositeRoutes.post('/batch', async (c) => {
  ...
  try {
    ...
    return c.json({
      ...
      context: {
        conversationId,
        continuationHint: failed.length > 0
...
async function processInParallel(requests, c, conversationId) {
  return Promise.all(
    requests.map((req, index) =>
      processRequest(req, c, conversationId, index)
    )
  );
}
...
async function processRequest(request, c, conversationId, index, previousContext = null) {
