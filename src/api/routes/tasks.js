...
tasksRoutes.get('/', async (c) => {
  try {
    ...
    const tasks = results.map(t => ({
      ...
      completed_at: t.completed_at ? new Date(t.completed_at * 1000).toISOString() : null
    }));
...
tasksRoutes.patch('/:taskId', async (c) => {
  try {
    ...
    const allowedFields = ['status', 'title', 'description', 'priority', 'assigned_service', 'result', 'error'];
    const setClauses = [];
    const params = [];
...
tasksRoutes.get('/:taskId', async (c) => {
  try {
    ...
    const formattedTask = {
      ...
      started_at: task.started_at ? new Date(task.started_at * 1000).toISOString() : null,
