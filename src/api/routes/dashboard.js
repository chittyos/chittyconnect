...
dashboard.get("/contexts", async (c) => {
  try {
    const db = c.env.DB;
    const { status, support_type, trust_level, limit = 50, offset = 0 } = c.req.query();
    ...
    // Get total count
    const countQuery = `SELECT COUNT(*) as total FROM context_entities WHERE 1=1` +
...
dashboard.get("/contexts/:id", async (c) => {
  try {
    ...
    `).bind(contextId, contextId).first();
  ...
  try {
    ...
    return c.json({
      ...
      data: {
        ...
        period: { days: parseInt(days), from: new Date(cutoff * 1000).toISOString() }
      }
    });
  } catch (error) {
...
dashboard.post("/contexts/:id/trust/adjust", async (c) => {
  try {
    ...
    if (!adjustment || !reason) {
      return c.json({ success: false, error: "adjustment and reason required" }, 400);
    }
    ...
    const newScore = Math.max(0, Math.min(100, context.trust_score + adjustment));
    ...
    await db.prepare(`
      ...
      WHERE id = ?
    `).bind(newScore, newLevel, context.id).run();

    // Log trust change
    const logId = crypto.randomUUID();
    const contentHash = await hashContent({ contextId, adjustment, reason, newScore });
    ...
    await db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(
      logId, context.id, context.chitty_id,
      context.trust_level, newLevel,
      context.trust_score, newScore,
      'manual_adjustment',
...
dashboard.get("/contexts/:id/decommission/preview", async (c) => {
  try {
    ...
    const ledgerEntries = await db.prepare(`
      SELECT COUNT(*) as count FROM context_ledger WHERE context_id = ?
    `).bind(context.id).first();

    const trustLogs = await db.prepare(`
    ...
    return c.json({
      ...
      data: {
        ...
        warnings: activeSessions?.count > 0
          ? [`${activeSessions.count} active sessions will be forcibly unbound`]
          : [],
        recommendation: context.status === 'active'
...
dashboard.post("/contexts/:id/decommission", async (c) => {
  try {
    ...
    if (!['archive', 'revoke'].includes(action)) {
      return c.json({ success: false, error: "action must be 'archive' or 'revoke'" }, 400);
    }
    ...
    await db.prepare(`
      ...
      WHERE id = ?
    `).bind(newStatus, context.id).run();

    // Log to ledger
    const ledgerId = crypto.randomUUID();
    const previousHash = await db.prepare(`
    ...
    await db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(
      ledgerId, context.id, context.chitty_id, 'system',
      'decision', JSON.stringify(payload), entryHash, previousHash?.hash || 'genesis'
    ).run();
...
dashboard.post("/contexts/:id/reactivate", async (c) => {
  try {
    ...
    if (context.status === 'revoked') {
      return c.json({ success: false, error: "Revoked contexts cannot be reactivated" }, 400);
    }

    if (context.status === 'active') {
      return c.json({ success: false, error: "Context is already active" }, 400);
    }
...
dashboard.post("/approvals/:id/approve", async (c) => {
  try {
    ...
    `).bind(JSON.stringify(payload), approvalId).run();
    ...
    if (payload.requested_trust_level) {
      await db.prepare(`
        ...
        WHERE id = ?
      `).bind(payload.requested_trust_level, request.context_id).run();
    }
...
dashboard.get("/team-candidates", async (c) => {
  try {
    ...
    if (support_types) {
      const types = support_types.split(',').map(t => `'${t.trim()}'`).join(',');
    ...
    if (required_competencies) {
      const required = required_competencies.split(',').map(c => c.trim().toLowerCase());
      candidates = candidates.filter(ctx => {
        const ctxCompetencies = ctx.competencies.map(c =>
          (typeof c === 'string' ? c : c.name || '').toLowerCase()
        );
        return required.some(r => ctxCompetencies.some(cc => cc.includes(r)));
      });
    }
    ...
    if (min_success_rate) {
      candidates = candidates.filter(ctx =>
        (ctx.success_rate || 0) >= parseFloat(min_success_rate)
      );
    }
    ...
    return c.json({
      ...
      data: {
        candidates,
        criteria: { support_types, min_trust_level, min_success_rate, required_competencies }
      }
    });
  } catch (error) {
...
dashboard.post("/projects/:projectId/bind", async (c) => {
  try {
    ...
    if (!context_ids || !Array.isArray(context_ids) || context_ids.length === 0) {
      return c.json({ success: false, error: "context_ids array required" }, 400);
    }
    ...
    for (const contextId of context_ids) {
      ...
      `).bind(
        ledgerId, context.id, context.chitty_id, 'system',
        'transaction', JSON.stringify(payload), entryHash, previousHash?.hash || 'genesis'
      ).run();
...
dashboard.get("/stats", async (c) => {
  try {
    ...
    const trustDist = await db.prepare(`
    ...
    return c.json({
      ...
      data: {
        overview: {
          total_contexts: statusCounts.results.reduce((sum, s) => sum + s.count, 0),
          active_contexts: statusCounts.results.find(s => s.status === 'active')?.count || 0,
          dormant_contexts: statusCounts.results.find(s => s.status === 'dormant')?.count || 0,
          archived_contexts: statusCounts.results.find(s => s.status === 'archived')?.count || 0,
...
function generateAlchemySuggestions({ support_type, trust_level, competencies, expertise, success_rate }) {
  const suggestions = [];

  // Base MCP tools for all contexts
  const baseTools = [
    { tool: 'chittyid_mint', reason: 'Identity management', confidence: 0.9 },
    { tool: 'chitty_services_status', reason: 'Ecosystem monitoring', confidence: 0.85 },
  ];

  // Support type specific suggestions
  const typeTools = {
    development: [
      { tool: 'github_pr_create', reason: 'Code workflow automation', confidence: 0.9 },
      { tool: 'chitty_evidence_ingest', reason: 'Development artifact tracking', confidence: 0.8 },
      { tool: 'neon_query', reason: 'Database operations', confidence: 0.75 },
    ],
    operations: [
      { tool: 'chitty_consciousness_snapshot', reason: 'Infrastructure monitoring', confidence: 0.95 },
      { tool: 'chitty_consciousness_heal', reason: 'Self-healing automation', confidence: 0.9 },
      { tool: 'cloudflare_kv_ops', reason: 'Cache management', confidence: 0.85 },
    ],
    legal: [
      { tool: 'chitty_case_create', reason: 'Case management', confidence: 0.95 },
      { tool: 'chitty_evidence_ingest', reason: 'Evidence management', confidence: 0.95 },
      { tool: 'notion_database_query', reason: 'Document research', confidence: 0.8 },
    ],
    research: [
      { tool: 'chitty_memory_recall', reason: 'Knowledge retrieval', confidence: 0.9 },
      { tool: 'web_search', reason: 'Information gathering', confidence: 0.85 },
      { tool: 'notion_create_page', reason: 'Research documentation', confidence: 0.8 },
    ],
    financial: [
      { tool: 'chitty_finance_connect_bank', reason: 'Banking integration', confidence: 0.9 },
      { tool: 'chitty_ledger_entry', reason: 'Financial tracking', confidence: 0.95 },
      { tool: 'chitty_evidence_ingest', reason: 'Financial document management', confidence: 0.85 },
    ],
    administrative: [
      { tool: 'notion_create_task', reason: 'Task management', confidence: 0.9 },
      { tool: 'google_calendar_event', reason: 'Scheduling', confidence: 0.85 },
      { tool: 'chitty_sync_trigger', reason: 'Cross-system sync', confidence: 0.8 },
    ],
  };
  ...
  if (trust_level >= 4) {
    suggestions.push(
      { tool: 'chitty_trust_adjust', reason: 'High-trust context can manage trust', confidence: 0.7 },
      { tool: 'credential_provision', reason: 'Credential management access', confidence: 0.8 },
    );
  }

  // Competency-based suggestions
  const competencyTools = {
    'code_review': { tool: 'github_pr_review', reason: 'Matches code review competency', confidence: 0.9 },
    'documentation': { tool: 'notion_create_page', reason: 'Matches documentation competency', confidence: 0.85 },
    'testing': { tool: 'test_runner', reason: 'Matches testing competency', confidence: 0.9 },
    'deployment': { tool: 'cloudflare_deploy', reason: 'Matches deployment competency', confidence: 0.85 },
  };
