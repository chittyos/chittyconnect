...
filesRoutes.post('/presign', async (c) => {
  try {
    ...
    if (!name || !session_id || !size) {
      return c.json({ error: 'invalid_request', message: 'name, session_id, and size required' }, 400)
    }
    ...
    return c.json({ error: 'presign_failed', message: String(err?.message || err) }, 500)
  }
})
...
filesRoutes.post('/commit', async (c) => {
  try {
    ...
    if (!r2_key || !session_id) return c.json({ error: 'invalid_request' }, 400)
    const sm = c.get('streaming')
    if (sm) {
      await sm.broadcast({ type: 'context.files.committed', sessionId: session_id, file: { r2_key, sha256, size, task_id } })
    }
...
filesRoutes.put('/upload/:token', async (c) => {
  try {
    ...
    return c.json({ error: 'upload_failed', message: String(err?.message || err) }, 500)
  }
})
...
filesRoutes.post('/upload', async (c) => {
  try {
    ...
    const { name, sha256, session_id, size, mime, content_base64 } = await c.req.json()
    if (!name || !session_id || !content_base64) return c.json({ error: 'invalid_request' }, 400)
    ...
    await env.FILES.put(key, bytes, { httpMetadata: { contentType: mime || 'application/octet-stream' } })

    const sm = c.get('streaming')
    if (sm) {
      await sm.broadcast({ type: 'context.files.uploaded', sessionId: session_id, file: { r2_key: key, sha256, size: size || bytes.length, name } })
    }
