...
export function registerDocumentRoutes(app) {
  ...
  app.get('/api/v1/documents/:documentId/download', async (c) => {
    try {
      ...
      const docService = new DocumentStorageService(c.env);
      const object = await docService.downloadDocument(documentId, verifiedChittyId);
  ...
  app.post('/api/v1/documents/:documentId/presigned-url', async (c) => {
    try {
      ...
      const docService = new DocumentStorageService(c.env);
      const url = await docService.getPresignedUrl(documentId, chittyId, expiresIn);
   ...
   */
  app.put('/api/v1/documents/multipart/:uploadId/part/:partNumber', async (c) => {
    try {
      ...
      const docService = new DocumentStorageService(c.env);
      const uploadedPart = await docService.uploadPart(uploadId, partNumber, data);
  ...
  app.post('/api/v1/documents/multipart/:uploadId/complete', async (c) => {
    try {
      ...
      const docService = new DocumentStorageService(c.env);
      const document = await docService.completeMultipartUpload(uploadId, parts);
